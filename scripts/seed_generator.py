...
from src.utils.logging import ts_print
...
def step0_series_expansion(pool: dict):
    ...
    if result.get("proposal"):
        ...
        if p["validation"]["passed"]:
            ts_print(f"  ✅ 自动通过: {p['rationale'][:80]}...")
...
def step1_generate_seeds(pool: dict, target: int, engine_status=None):
    ...
    for series_key in all_series_keys():
        if series_key not in pool:
            ts_print(f"  ⚠️ {series_key} 不在 seed_pool，跳过")
            continue
        ...
        ts_print(f"  🌱 {series_key}: pending={pending_count}, 需补 {needed} 个")
        ...
        store.write(pool)
        ts_print(f"    ✅ +{accepted} 种子 (pending 总计 {len([s for s in seeds if s.get('status')=='pending'])}）")
...
def main():
    ...
    ts_print(f"🌱 LocalLLM-IP-Factory · 阶段一 (目标 pending ≥ {args.target})")
    ts_print()
